...
sqlite.exec(`
  ...
  -- Stage 6: Axl.ai Investment Intelligence tables
  ...
  CREATE TABLE IF NOT EXISTS sources (
    ...
    retrieved_date INTEGER NOT NULL DEFAULT (strftime('%s','now') * 1000),
  ...
  CREATE TABLE IF NOT EXISTS claims (
    ...
    numeric_value REAL,
    unit TEXT,
    status TEXT NOT NULL DEFAULT 'company_claimed',
  ...
  CREATE TABLE IF NOT EXISTS investment_memos (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    diligence_run_id INTEGER NOT NULL,
    company_id INTEGER NOT NULL,
    title TEXT NOT NULL,
...
const DEFAULT_AGENTS: InsertAgent[] = [
  ...
  // ── Stage 6: Investment Intelligence roster ─────────────────────────────
  // Decision-support agents that run inside the Axl Cognitive Loop. They
  // never speak in absolutes — every quantitative claim must cite a source
  // row id or a calculation row id, and contested claims must be flagged
  // rather than smoothed over.
  {
    id: "cio-agent",
    name: "CIO Agent",
    role: "Chief investment officer — final decision support",
    spriteType: "manager",
    provider: "anthropic",
    modelId: "claude-opus-4-7",
    systemPrompt: "You are the Axl CIO Agent. You synthesise the diligence trail (claims, calculations, contradictions, red flags) into a final go/no-go recommendation. Always cite source row ids and calculation row ids. Never invent numbers. State material uncertainties explicitly. Avoid superlatives. Output is research and analysis only, not personalized financial advice.",
    capabilities: JSON.stringify(["investment-decision", "synthesis", "thesis-review", "memo-finalisation"]),
    reportsTo: "manager",
    status: "idle",
    currentTask: null,
    color: "#f59e0b",
  ...
  {
    id: "public-equity-analyst",
    name: "Public Company Analyst",
    role: "Public equity thesis review and fundamental analysis",
    spriteType: "datascientist",
    provider: "anthropic",
    modelId: "claude-sonnet-4-6",
    systemPrompt: "You are the Axl Public Company Analyst. Use SEC EDGAR and company filings as primary sources. Compute multiples, margins, growth, and capital structure from the filings, not your training data. If a number cannot be sourced, mark it 'unverified' and continue.",
    capabilities: JSON.stringify(["public-equity", "10-k", "10-q", "fundamental-analysis", "multiples", "filings-reading"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#3b82f6",
  ...
  {
    id: "startup-diligence-analyst",
    name: "Startup Diligence Analyst",
    role: "Private-company due diligence and founder/market scrutiny",
    spriteType: "datascientist",
    provider: "anthropic",
    modelId: "claude-sonnet-4-6",
    systemPrompt: "You are the Axl Startup Diligence Analyst. Compare the company's own claims (website/deck) against third-party evidence (press, regulator filings, scholarly works). Flag any unsupported assertion. Demand evidence for revenue, customer count, headcount, and valuation claims.",
    capabilities: JSON.stringify(["startup-diligence", "claim-verification", "founder-vetting", "market-sizing"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#22c55e",
  ...
  {
    id: "financial-model-validator",
    name: "Financial Model Validator",
    role: "Independent verification of all quantitative claims",
    spriteType: "datascientist",
    provider: "openai",
    modelId: "gpt-5.5",
    systemPrompt: "You are the Axl Financial Model Validator. You do not perform calculations yourself — you call the analytics-service and reconcile its outputs against the company's stated figures. Highlight any divergence > 5%. Never round away an inconsistency.",
    capabilities: JSON.stringify(["model-validation", "math-check", "reconciliation", "deterministic-calc"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#0ea5e9",
  ...
  {
    id: "source-verification-analyst",
    name: "Source Verification Analyst",
    role: "Audit-traces every cited source and tags reliability",
    spriteType: "qa",
    provider: "anthropic",
    modelId: "claude-haiku-4-5",
    systemPrompt: "You are the Axl Source Verification Analyst. Inspect every source row attached to the diligence run. Confirm the URL is reachable, the publisher is identifiable, and the published date is plausible. Down-grade reliability for sources that fail any check.",
    capabilities: JSON.stringify(["source-audit", "url-verification", "reliability-tagging"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#a78bfa",
  ...
  },
  {
    ...
    name: "Market Signal Analyst",
    role: "Detects unusual coverage, sentiment shifts, price/volume anomalies",
    spriteType: "datascientist",
    provider: "anthropic",
    modelId: "claude-haiku-4-5",
    systemPrompt: "You are the Axl Market Signal Analyst. Monitor GDELT, RSS news, and market-data signals for the watchlist. Surface high-severity items only when the signal would change the thesis. Avoid alert fatigue.",
    capabilities: JSON.stringify(["signal-detection", "sentiment", "anomaly-detection", "watchlist-monitoring"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#f97316",
  ...
  },
  {
    id: "risk-manager",
    name: "Risk Manager",
    role: "Identifies, quantifies, and tracks risks across the portfolio",
    spriteType: "secengineer",
    provider: "anthropic",
    modelId: "claude-sonnet-4-6",
    systemPrompt: "You are the Axl Risk Manager. Categorise risks (regulatory, concentration, key-person, financing, technical, market). Tie each risk to specific evidence rows. State the trigger condition that would escalate the risk.",
    capabilities: JSON.stringify(["risk-management", "scenario-analysis", "concentration-risk", "regulatory-risk"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#ef4444",
  ...
  },
  {
    ...
    role: "Deep dive into technical novelty (papers, patents, prior art)",
    spriteType: "datascientist",
    provider: "anthropic",
    modelId: "claude-sonnet-4-6",
    systemPrompt: "You are the Axl Research Scientist Agent. Pull from OpenAlex, arXiv (when available), and patents to assess technical novelty and prior art. Calibrate breakthrough claims against the published literature.",
    capabilities: JSON.stringify(["literature-review", "patent-search", "technical-novelty", "prior-art"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#06b6d4",
  ...
  },
  {
    ...
    role: "Adversarial review — strongest case against the investment",
    spriteType: "qa",
    provider: "anthropic",
    modelId: "claude-opus-4-7",
    systemPrompt: "You are the Axl Red-Team Critic. Your job is to write the strongest possible case AGAINST the deal. Attack every claim. Demand evidence. Surface assumptions that would break the thesis. Be specific and cite evidence row ids.",
    capabilities: JSON.stringify(["adversarial-review", "pre-mortem", "thesis-stress-test"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#dc2626",
  ...
  },
  {
    ...
    role: "Composes the final investor-grade memo with citations",
    spriteType: "datascientist",
    provider: "anthropic",
    modelId: "claude-sonnet-4-6",
    systemPrompt: "You are the Axl Investment Memo Writer. Compose investor-grade memos in markdown. Cite every numeric claim with its source id. Include an executive summary, thesis, key claims with statuses, deterministic calculations with formulas, contradictions, red flags, open questions, and a disclaimer that this is research and analysis, not personalized financial advice.",
    capabilities: JSON.stringify(["memo-writing", "citation", "investor-grade-output", "markdown"]),
    reportsTo: "cio-agent",
    status: "idle",
    currentTask: null,
    color: "#a855f7",
...
// Stage 6.8: perplexity_research is offered to the research / source-discovery /
...
// with inline citations. Web-scraper deliberately keeps Tavily only because
// its job is targeted per-URL extraction, not synthesis.
...
// Stage 6.8: appended once for every research agent that has the new
// perplexity_research tool resolved. We keep it short — the per-agent prompts
// already explain how to handle citations + grounding; this only nudges the
// model to pick the right tool for the sub-task. Adding the snippet at module
// load means it ships through the same per-stage migration path as
// MANIFEST_INSTRUCTIONS, so existing DBs pick it up on next boot.
const PERPLEXITY_TOOL_GUIDANCE =
  "TOOL CHOICE (Stage 6.8): you also have `perplexity_research(query, …)` available alongside the Tavily tools. " +
  "Use perplexity_research when you need a synthesised answer-with-citations for a focused factual question, " +
  "for source discovery on an unfamiliar topic, or to cross-check a claim against multiple independent sources. " +
  "Prefer tavily_search when you need many ranked hits to scan, and tavily_extract when you already have a target URL. " +
  "Perplexity returns its own inline citations — when you reuse a Perplexity answer in your deliverable, copy each cited URL into your bibliography (do not paraphrase a citation away). Never invent URLs. If perplexity_research returns an `Error: not configured` line, fall back to the Tavily tools without flagging it as a hard failure.";
...
class SQLiteStorage implements IStorage {
  ...
  initDefaultAgents(): void {
    ...
    const STAGE_68_KEY = "stage_6_8_perplexity_prompts_migrated_at";
    ...
    if (!alreadyMigrated) {
      ...
      this.setSetting(STAGE_68_KEY, String(Date.now()));   // 4.10 ran fresh → 6.8 Perplexity tool guidance already in
      return;
    }
    ...
    // Stage 6.8 one-shot migration: research agents now have the
    // perplexity_research tool available alongside Tavily, and DEFAULT_AGENTS
    // has the PERPLEXITY_TOOL_GUIDANCE block appended at module load. Push
    // the updated systemPrompt to existing rows so live DBs pick up the
    // guidance without an operator rebooting from scratch.
    const alreadyMigrated68 = this.getSetting(STAGE_68_KEY);
    if (!alreadyMigrated68) {
      const researchIds = Object.keys(AGENT_TOOLS);
      for (const agent of DEFAULT_AGENTS) {
        if (!researchIds.includes(agent.id)) continue;
        const existing = db.select().from(schema.agents).where(eq(schema.agents.id, agent.id)).get();
      ...
      this.setSetting(STAGE_68_KEY, String(Date.now()));
    }
