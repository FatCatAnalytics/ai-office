...
import { Settings, Key, CheckCircle2, AlertTriangle, Save, Eye, EyeOff, Loader2, Zap, Globe, Sparkles, RefreshCw, Boxes, Star, DollarSign, Database, Trash2, ChevronLeft, ChevronRight } from "lucide-react";
...
// ─── Stage 5.x.12: Database Browser ──────────────────────────────────
// Read+delete for the 11 whitelisted tables. The server hard-codes the
// table list in routes.ts — no client SQL is ever sent. Pagination uses
// the limit+offset query params. Delete prompts confirm() before firing.
...
function DatabaseBrowser() {
  ...
  const { data: tables = [], refetch: refetchTables } = useQuery<TableInfo[]>({
    ...
    queryFn: () => apiRequest("GET", "/api/db/tables").then(r => r.json()),
  });

  const { data: rowsData, refetch: refetchRows, isFetching: rowsLoading } = useQuery<RowsResponse>({
    queryKey: ["/api/db/tables", selected, page],
    enabled: !!selected,
    queryFn: () => apiRequest(
      "GET",
      `/api/db/tables/${encodeURIComponent(selected!)}/rows?limit=${PAGE_SIZE}&offset=${page * PAGE_SIZE}`,
    ).then(r => r.json()),
  });

  const onDelete = async (id: unknown) => {
    if (!selected) return;
    if (!window.confirm(`Delete row ${String(id)} from ${selected}? This cannot be undone.`)) return;
    try {
      await apiRequest("DELETE", `/api/db/tables/${encodeURIComponent(selected)}/rows/${encodeURIComponent(String(id))}`);
      await refetchRows();
      await refetchTables();
    } catch (e) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-3">
      {/* Tables list */}
      <div className="md:w-56 flex-shrink-0 space-y-1" data-testid="db-tables-list">
        {tables.map(t => (
          <button
            key={t.name}
            onClick={() => { setSelected(t.name); setPage(0); }}
            ...
            <span className="text-[10px] text-slate-600 font-mono">{t.rowCount}</span>
          </button>
        ))}
      </div>

      {/* Rows pane */}
      <div className="flex-1 min-w-0 rounded-xl border border-slate-800 bg-slate-900/40">
        {!selected ? (
          ...
          <>
            <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800">
              <div className="flex items-center gap-2 text-xs">
                <span className="font-mono text-slate-300">{rowsData.table}</span>
                <span className="text-slate-600">· {rowsData.total} rows</span>
                {rowsLoading && <Loader2 size={11} className="animate-spin text-slate-500"/>}
              </div>
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  disabled={(page + 1) * PAGE_SIZE >= rowsData.total}
                  className="p-1 text-slate-500 hover:text-slate-300 disabled:opacity-30">
            ...
            <div className="overflow-x-auto custom-scroll">
              {rowsData.rows.length === 0 ? (
                <div className="flex items-center justify-center h-32 text-xs text-slate-600">empty</div>
              ) : (
                <table className="w-full text-[11px]">
                  <thead>
                    <tr className="border-b border-slate-800 bg-slate-900/80">
                      {rowsData.columns.map(c => (
                        <th
                          key={c}
                          className="text-left px-2 py-1.5 text-slate-500 font-semibold uppercase tracking-wider whitespace-nowrap">
                          {c}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {rowsData.rows.map((row, i) => {
                      const pkValue = row[rowsData.primaryKey];
                      return (
                        <tr
                          key={`${pkValue}-${i}`}
                          className="border-b border-slate-800/60 last:border-0 hover:bg-slate-800/30">
                          {rowsData.columns.map(c => (
                            <td key={c} className="px-2 py-1.5 font-mono text-slate-300 align-top max-w-[280px]">
                          ...
                          <td className="px-2 py-1.5">
                            <button
                              ...
                              title="Delete row"
                              data-testid={`delete-row-${pkValue}`}>
                              <Trash2 size={12}/>
                            </button>
                          </td>
                        </tr>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950">
      ...
      <div className="flex-1 overflow-y-auto custom-scroll px-6 py-5 space-y-6">
        ...
        <div>
          <h3 className="text-xs text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
            <Database size={11}/> Database Browser
          </h3>
          <DatabaseBrowser/>
        </div>
