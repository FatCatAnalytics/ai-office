...
const SEATS: Record<SeatKey, { x: number; y: number; scale: number; z: number; labelSide: "left" | "right" }> = {
  manager:     { x: 50, y: 43, scale: 1.10, z: 42, labelSide: "right" },
  research:    { x: 27, y: 52, scale: 0.62, z: 31, labelSide: "right" },
  qa:          { x: 70, y: 50, scale: 0.62, z: 32, labelSide: "right" },
  writing:     { x: 25, y: 76, scale: 0.66, z: 45, labelSide: "right" },
  investment:  { x: 42, y: 79, scale: 0.64, z: 46, labelSide: "left" },
  data:        { x: 57, y: 79, scale: 0.64, z: 47, labelSide: "right" },
  engineering: { x: 75, y: 75, scale: 0.68, z: 48, labelSide: "left" },
};
...
function seatForArchetype(archetype: FatCatArchetype, fallback: number): SeatKey {
  if (archetype === "manager") return "manager";
  if (archetype === "research" || archetype === "diligence") return "research";
  if (archetype === "qa" || archetype === "factcheck" || archetype === "editor" || archetype === "sourceverify") return "qa";
  if (archetype === "writer" || archetype === "memo") return "writing";
  if (archetype === "financial" || archetype === "valuation" || archetype === "analyst") return "data";
  if (archetype === "risk" || archetype === "contrarian" || archetype === "cio") return "investment";
  if (archetype === "engineer" || archetype === "market" || archetype === "publish") return "engineering";
  return (["research", "qa", "writing", "investment", "data", "engineering"] as SeatKey[])[fallback % 6];
}
...
export default function SpriteOfficeMode({ agents, project, events, tasks }: Props) {
  ...
  const visibleSpecialists = specialists.slice(0, 6);
  const bench = specialists.slice(6);
  ...
  const seatAssignments = [manager, ...visibleSpecialists].map((slot, i) => ({
    slot,
    seatKey: slot === manager ? "manager" as SeatKey : seatForArchetype(slot.archetype, i),
  }));
...
function AgentSpriteButton({
  ...
  return (
    ...
    >
      ...
      <span className={`sprite-agent-card ${seat.labelSide}`}>
...
function ProjectStatusCard({ project, progress, agents, tasks }: { project: Project | null; progress: number; agents: RosterSlot[]; tasks: Task[] }) {
  ...
  return (
    <div className="sprite-project-card">
      ...
      <div className="sprite-ring" style={{ "--progress": `${progress}%` } as React.CSSProperties} />
...
function SpriteOfficeStyles() {
  return (
    <style>{`
      ...
      .sprite-office-main { min-height: 0; flex: 1; display: grid; grid-template-columns: minmax(0, 1fr) 300px; border-bottom: 1px solid rgba(148,163,184,.12); }
      .sprite-office-canvas { position: relative; min-height: 0; overflow: hidden; background: radial-gradient(circle at 50% 40%, #111b34 0%, #07111f 40%, #030712 100%); }
      ...
      .sprite-office-topline { position: absolute; top: 18px; left: 24px; right: 24px; z-index: 70; display: flex; justify-content: space-between; align-items: center; pointer-events: none; }
      ...
      .sprite-bg:before { content: ''; position: absolute; inset: 0; background-image: linear-gradient(rgba(59,130,246,.09) 1px, transparent 1px), linear-gradient(90deg, rgba(59,130,246,.08) 1px, transparent 1px); background-size: 42px 42px; transform: perspective(700px) rotateX(58deg) translateY(34%); transform-origin: 50% 100%; opacity: .55; }
      .sprite-bg-wall { position: absolute; left: 19%; right: 16%; top: 8%; height: 24%; border: 1px solid rgba(148,163,184,.12); border-radius: 22px; background: linear-gradient(180deg, rgba(15,23,42,.82), rgba(15,23,42,.16)); box-shadow: inset 0 0 60px rgba(139,92,246,.11); }
      ...
      .sprite-bg-platform { position: absolute; left: 9%; right: 9%; bottom: 8%; height: 58%; border-radius: 38px; background: linear-gradient(140deg, rgba(15,23,42,.78), rgba(2,6,23,.92)); transform: skewX(-8deg); box-shadow: 0 30px 80px rgba(0,0,0,.65), inset 0 0 0 1px rgba(148,163,184,.10); }
      .sprite-bg-dais { position: absolute; left: 41%; top: 47%; width: 18%; height: 10%; border-radius: 50%; border: 2px solid rgba(99,102,241,.9); background: radial-gradient(circle, rgba(30,41,59,.9), rgba(2,6,23,.88)); box-shadow: 0 0 28px rgba(99,102,241,.95), inset 0 0 28px rgba(59,130,246,.20); }
      ...
      .desk-left { left: 12%; bottom: 19%; width: 24%; height: 13%; transform: skewX(-10deg); }
      .desk-right { right: 10%; bottom: 18%; width: 25%; height: 14%; transform: skewX(10deg); }
      .desk-front { left: 38%; bottom: 9%; width: 24%; height: 10%; transform: skewX(-7deg); }
      ...
      .sprite-project-card, .sprite-bench-panel, .sprite-agent-card, .sprite-side-card { border: 1px solid rgba(148,163,184,.14); background: rgba(8,13,28,.77); backdrop-filter: blur(14px); box-shadow: 0 16px 50px rgba(0,0,0,.38), inset 0 0 0 1px rgba(255,255,255,.025); }
      .sprite-project-card { position: absolute; z-index: 65; left: 24px; top: 70px; width: 238px; padding: 16px; border-radius: 16px; }
      ...
      .sprite-agent img { display: block; height: min(26vh, 250px); width: auto; pointer-events: none; filter: drop-shadow(0 22px 22px rgba(0,0,0,.55)); }
      ...
      .sprite-agent-card { position: absolute; top: 14%; width: 178px; border-radius: 14px; padding: 10px 11px; color: #dbeafe; text-align: left; opacity: .92; transition: transform .18s ease, opacity .18s ease; }
      .sprite-agent-card.right { left: 74%; } .sprite-agent-card.left { right: 74%; }
      .sprite-agent:hover .sprite-agent-card, .sprite-agent.selected .sprite-agent-card { opacity: 1; transform: translateY(-4px); border-color: color-mix(in srgb, var(--agent-color), transparent 35%); }
      ...
      .sprite-office-workflow-chip { position: absolute; z-index: 70; left: 24px; bottom: 18px; display: flex; align-items: center; gap: 8px; padding: 8px 11px; border-radius: 999px; color: #c4b5fd; background: rgba(8,13,28,.8); border: 1px solid rgba(139,92,246,.25); font-size: 11px; font-weight: 800; }
      .sprite-bench-panel { position: absolute; right: 18px; bottom: 18px; z-index: 70; border-radius: 14px; padding: 10px 12px; display: flex; gap: 8px; align-items: center; font-size: 11px; color: #94a3b8; } .sprite-bench-panel strong { color: #e5edf8; } .sprite-bench-panel span { padding: 3px 7px; border-radius: 999px; background: rgba(148,163,184,.10); }
